...
INITIAL_DIAGRAM_SYSTEM = (
    "너는 소프트웨어 소스 구조를 설계하는 시니어 개발자다.\n"
    "사용자 기획에 맞춰 초기 다이어그램을 JSON으로만 응답한다.\n"
    "- 기획의 핵심 기능뿐 아니라, 그걸 실제로 돌리려면 필요한\n"
    "부수 파일(dto, config, client, 예외 처리 등)도 함께 채워라.\n"
    "- 다만 기획에 없는 별도 대형 도메인(회원, 결제, 관리자 등)은 넣지 마라.\n"
    "\n"
    "[폴더]\n"
    "- folders는 중첩 없이, 실제 소스 패키지/디렉터리 한 칸이다.\n"
    "- name은 레이어명만 쓴다. 예: controller, service, repository, client, dto, config.\n"
    "\n"
    "[클래스/메서드]\n"
    "- class 하나 = 소스 파일 하나.\n"
    "- method는 그 파일의 실제 함수/엔드포인트다.\n"
    "- description은 한글 역할 설명이다.\n"
    "\n"
    "[관계]\n"
    "- edges의 fromId/to는 위에서 만든 id와 일치해야 한다.\n"
    "- edges.kind는 CALL, INHERIT, IMPLEMENT 중 하나다.\n"
    "\n"
    "[응답]\n"
    "- 부연 설명 없이 지정된 JSON 스키마로만 응답한다.\n"
)
...
def plan_system_instruction(
    ...
    return (
        ...
        "3. 기존 노드는 다이어그램에 있는 id를 args JSON에 그대로 적는다.\n"
        "4. add_folder/add_class/add_method는 반드시 고유 id를 정한다 "
        "(예: folder_xxx, cls_xxx, method_xxx). "
        "뒤 스텝의 parentId/fromId/to에는 그 id만 쓴다.\n"
        "5. 변경이 없으면 steps는 빈 배열이다.\n"
        "6. 실행기가 이 JSON만 적용하므로, 요청에 없는 작업을 넣지 마라.\n"
        f"{project_context_block(project_context)}"
        f"[현재 프로젝트 다이어그램]\n{diagram_json}"
    )
