...
from utils.diagram_helper import (
    serialize_diagram,
    build_diagram_agent_state,
    build_code_agent_state,
)
...
def generate_file_tree(request: FileTreeRequest) -> FileStructureResponse:
    ...
    initial_state = build_code_agent_state(
        mode="file_tree",
        system_instruction=file_tree_system_instruction(request.targetFramework),
        user_contents=[file_tree_user_message(diagram_json)]
    )

    try:
        result = code_agent_app.invoke(initial_state)
        ...
        return FileStructureResponse(**result["result"])
...
def generate_single_code(
    ...
    initial_state = build_code_agent_state(
        mode="single_code",
        system_instruction=single_code_system_instruction(request.targetFilePath),
        user_contents=[
            single_code_user_message(
                diagram_json,
    ...
    try:
        ...
        if not result.get("result"):
            raise HTTPException(
                status_code=502,
                detail=f"[{request.targetFilePath}] 코드 생성에 실패했습니다."
            )

        return SingleCodeGenerationResponse(**result["result"])
